...
async function main() {
  ...
  const configs = [
    ...
    { key: "footer_disclaimer", value: "Las imágenes y planos son una representación artística del diseño del proyecto y están sujetas a cambios sin previo aviso. Los materiales, acabados, colores, texturas y detalles representados en las imágenes son solo para ambientación, no comprometen al constructor y están sujetos a cambios sin previo aviso." },
  ];
  ...
  const lagos = await prisma.project.upsert({
    ...
    create: {
      ...
      descripcionLarga: "Vive en un lugar donde la naturaleza es tu vecina más cercana. Lagos del Palmar es un proyecto campestre de gran escala pensado para quienes buscan retiro, tranquilidad y conexión con la naturaleza, sin sacrificar accesibilidad y servicios.",
      lotes: 554,
      areaDesde: 310,
      reserva: "$1.000.000",
  ...
  for (const step of steps) {
    await prisma.comoComprarStep.create({ data: step }).catch(() => {});
  }
