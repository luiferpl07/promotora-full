...
// The location field holds a full descriptive sentence ("Sincelejo, Sucre.
// Acceso pavimentado..."); only the part before the first period is an
// actual place name, so that's what the filter groups and displays by.
function ciudadFromUbicacion(texto: string): string {
  return texto.split(".")[0].trim();
}

export default function ProyectosGallery({ proyectos }: { proyectos: Project[] }) {
  const container = useRef<HTMLDivElement>(null);
  const [ubicacion, setUbicacion] = useState<string>("todas");

  const ubicaciones = useMemo(() => {
    const set = new Set(
      proyectos.map(p => p.ubicacionTexto).filter((u): u is string => !!u).map(ciudadFromUbicacion)
    );
    return Array.from(set);
  }, [proyectos]);

  const visibles = ubicacion === "todas"
    ? proyectos
    : proyectos.filter(p => p.ubicacionTexto && ciudadFromUbicacion(p.ubicacionTexto) === ubicacion);
  ...
  }, { scope: container, dependencies: [visibles.length, ubicacion] });
        ...
        <section className="bg-[var(--color-pf-bg)] text-[var(--color-pf-dark)] py-[clamp(100px,15vw,220px)] relative rounded-t-[40px] md:rounded-t-[80px] shadow-2xl">
          <div className="max-w-[1500px] mx-auto px-[5%]">
            ...
            {ubicaciones.length > 1 && (
              <div className="flex flex-wrap items-center gap-3 mb-[80px] md:mb-[120px]">
                <span className="text-[11px] uppercase tracking-[0.2em] text-[var(--color-pf-dark)]/40 font-mono mr-2">Ubicación</span>
                <button
                  onClick={() => setUbicacion("todas")}
                  className={`px-5 py-2.5 rounded-full text-[11px] uppercase tracking-[0.15em] font-semibold transition-colors border ${ubicacion === "todas" ? "bg-[var(--color-pf-navy)] text-white border-[var(--color-pf-navy)]" : "border-black/15 text-[var(--color-pf-dark)]/70 hover:border-[var(--color-pf-navy)]"}`}
                >
                  Todas
                </button>
                {ubicaciones.map(u => (
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-full text-[11px] uppercase tracking-[0.15em] font-semibold transition-colors border ${ubicacion === u ? "bg-[var(--color-pf-navy)] text-white border-[var(--color-pf-navy)]" : "border-black/15 text-[var(--color-pf-dark)]/70 hover:border-[var(--color-pf-navy)]"}`}
                  >
                    {u}
                  </button>
                ))}
            ...
            <div className="grid gap-[150px] md:gap-[250px] relative">
                    ...
                    <Link href={`/proyectos/${p.slug}`} className="block relative">
                      ...
                      <div className={`mt-10 md:mt-16 grid grid-cols-1 md:grid-cols-12 gap-8 items-start ${align === "right" ? "md:pl-12" : ""}`}>
                        <div className="md:col-span-7">
                          ...
                          {p.ubicacionTexto && (
                            <p data-reveal className="text-[11px] uppercase tracking-[0.15em] text-[var(--color-pf-gold)] mt-3 font-mono">{p.ubicacionTexto}</p>
                          )}
              ...
              {visibles.length === 0 && (
                <div className="text-center py-20 text-[var(--color-pf-dark)]/50">No hay proyectos en esta ubicación.</div>
              )}
            </div>
          </div>
        </section>
