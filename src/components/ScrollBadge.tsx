...
export default function ScrollBadge() {
  ...
  useEffect(() => {
    const updateScrollProgress = () => {
      if (scrollProgressRef.current) {
        ...
        const progress = max > 0 ? Math.min(100, Math.max(0, Math.round((current / max) * 100))) : 0;
  ...
  return (
    <div className="fixed top-24 left-8 md:top-32 md:left-12 z-50 text-[var(--color-pf-gold)] drop-shadow-[0_1px_4px_rgba(0,0,0,0.35)] flex flex-col items-center gap-6 pointer-events-none">
      <div className="relative w-28 h-28 md:w-36 md:h-36">
        <svg viewBox="0 0 100 100" className="w-full h-full text-current animate-[spin_15s_linear_infinite]">
          <path id="scrollBadgeCirclePath" d="M 50, 50 m -35, 0 a 35,35 0 1,1 70,0 a 35,35 0 1,1 -70,0" fill="none" />
          <text fontSize="10" letterSpacing="4.5" fill="currentColor" className="font-mono uppercase">
            <textPath href="#scrollBadgeCirclePath" startOffset="0%">PROMOTORAS • FULL • </textPath>
          </text>
        </svg>
